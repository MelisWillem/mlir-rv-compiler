...
// CHECK:   %2 = "rvir.ADDI"() <{imm = 1 : i32}> : () -> !rvir.reg<5>
// CHECK:   "rvir.BEQ"(%1, %2)[^bb2] : (!rvir.reg<4>, !rvir.reg<5>) -> ()
// CHECK: ^bb1(%3: !rvir.reg<10>):  // pred: ^bb2
// CHECK:   return %3 : !rvir.reg<10>
// CHECK: ^bb2:  // pred: ^bb0
// CHECK:   %4 = "rvir.ADDI"() <{imm = 2 : i32}> : () -> !rvir.reg<4>
// CHECK:   "rvir.BEQ"(%arg1, %4)[^bb1] : (!rvir.reg<12>, !rvir.reg<4>) -> ()
// CHECK: }
...
module {
  func.func @foo(%arg0: !rvir.reg, %arg1: !rvir.reg) -> !rvir.reg {
    ...
    %2 = "rvir.ADDI"() <{imm = 1 : i32}> : () -> !rvir.reg
    "rvir.BEQ"(%1, %2)[^bb2] : (!rvir.reg, !rvir.reg) -> ()
  ^bb1(%3: !rvir.reg):  // pred: ^bb2
    return %3 : !rvir.reg
  ^bb2:  // pred: ^bb0
    %4 = "rvir.ADDI"() <{imm = 2 : i32}> : () -> !rvir.reg
    "rvir.BEQ"(%arg1, %4)[^bb1] : (!rvir.reg, !rvir.reg) -> ()
  }
}
